    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} aria-label="Close dialog" className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 text-lg">
            ✕
          </button>
        </div>
        ...
        {diff.urgencyDelta !== 0 && (
          <div className="mb-4 p-3 rounded-lg bg-slate-50 dark:bg-slate-900 border dark:border-slate-700 text-sm">
            <span className="font-medium text-slate-700 dark:text-slate-200">Urgency score change: </span>
            <span className={diff.urgencyDelta > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}>
              {diff.urgencyDelta > 0 ? '+' : ''}
              {diff.urgencyDelta}
            </span>
          </div>
        ...
        {diff.scoreChanges.length > 0 && (
          <div className="mb-4">
            <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">Domain Score Changes</h3>
            <div className="space-y-1">
              {diff.scoreChanges.map((sc) => (
                <div key={sc.domain} className="flex items-center gap-3 text-sm">
                  ...
                  <span className={sc.newScore > sc.oldScore ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}>
                    {sc.newScore}
                  </span>
                </div>
              ))}
          ...
          <div className="mb-4">
            ...
            <ul className="space-y-1">
              {diff.addedFindings.map((f, i) => (
                <li key={i} className="text-sm text-slate-700 dark:text-slate-200 bg-green-50 dark:bg-green-900/30 px-3 py-1.5 rounded">
                  <span className="text-xs text-green-600 dark:text-green-400 font-medium mr-2">{f.domain}</span>
          ...
          <div className="mb-4">
            ...
            <ul className="space-y-1">
              {diff.removedFindings.map((f, i) => (
                <li key={i} className="text-sm text-slate-700 dark:text-slate-200 bg-red-50 dark:bg-red-900/30 px-3 py-1.5 rounded">
                  <span className="text-xs text-red-600 dark:text-red-400 font-medium mr-2">{f.domain}</span>
                  {f.title ?? f.description ?? 'Unnamed finding'}
                </li>
              ))}
            </ul>
        ...
        {!diff.hasChanges && (
          <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-4">No changes between versions.</p>
        )}
