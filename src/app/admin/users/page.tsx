  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{USERS_PAGE.TITLE}</h1>
          <p className="text-sm text-slate-400 mt-0.5">{USERS_PAGE.DESCRIPTION}</p>
        </div>
      ...
      {data.loading ? (
        <p className="text-sm text-slate-400">{USERS_PAGE.LOADING}</p>
      ) : (
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-4 pt-2">
          <button
            ...
            className="text-sm px-3 py-1.5 rounded-md border border-slate-300 text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-50"
          >
